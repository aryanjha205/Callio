(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))i(n);new MutationObserver(n=>{for(const a of n)if(a.type==="childList")for(const r of a.addedNodes)r.tagName==="LINK"&&r.rel==="modulepreload"&&i(r)}).observe(document,{childList:!0,subtree:!0});function t(n){const a={};return n.integrity&&(a.integrity=n.integrity),n.referrerPolicy&&(a.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?a.credentials="include":n.crossOrigin==="anonymous"?a.credentials="omit":a.credentials="same-origin",a}function i(n){if(n.ep)return;n.ep=!0;const a=t(n);fetch(n.href,a)}})();const K="callio_access_token",je="https://callio-backend-nnas.onrender.com",C={token:localStorage.getItem(K)||null,user:null,setToken(s){this.token=s,s?localStorage.setItem(K,s):localStorage.removeItem(K)},logout(){this.token=null,this.user=null,localStorage.removeItem(K)}};async function B(s,e={}){const t={"Content-Type":"application/json",...e.headers||{}};C.token&&(t.Authorization=`Bearer ${C.token}`);const i={...e,headers:t},n=s.startsWith("http")?s:`${je}${s}`;try{const a=await fetch(n,i);if(a.status===401)throw C.logout(),window.location.hash="#/auth",new Error("Unauthorized session. Please log in.");const r=await a.json();if(!a.ok)throw new Error(r.detail||r.message||"API request failed");return r}catch(a){throw console.error(`API Error [${n}]:`,a),a}}function E(s,e="info"){const t=document.getElementById("toast-container");if(!t)return;const i=e==="success"?"bg-success":e==="danger"?"bg-danger":e==="warning"?"bg-warning text-dark":"bg-primary",n=e==="success"?"bi-check-circle-fill":e==="danger"?"bi-exclamation-octagon-fill":"bi-info-circle-fill",a=document.createElement("div");a.className=`toast align-items-center text-white ${i} border-0 show shadow-lg mb-2`,a.role="alert",a.innerHTML=`
    <div class="d-flex">
  ...
  `,(a=document.getElementById("btn-accept-incoming"))==null||a.addEventListener("click",async()=>{i.innerHTML="",z("connecting",t,e,s),await _.acceptCall(s,e,t)}),(r=document.getElementById("btn-reject-incoming"))==null||r.addEventListener("click",()=>{i.innerHTML="",_.rejectCall(s)})}function Ye(s){let e=!0;function t(){var i,n,a;s.innerHTML=`
